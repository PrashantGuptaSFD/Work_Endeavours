...
function initRAFDiscountRule1(lines,conn, objQuote, allLines){
	...
	.then(function(results) {		
		...
		lines.forEach(function(line) {
			...
			reinitializeDiscountsOnLineItemRAF(line,'No RAF Discount Applied, No match found in Table');
  ...
  conn.query(query)
	.then(function(results) {		
	  ...
	  lines.forEach(function(line) {
		...
		reinitializeDiscountsOnLineItem(line,'No Customer Discount Applied, No match found in Table');
		var obj;
		var log = '';
...
//discount : discount to be applied
//type : type of discount applied
function reinitializeDiscountsOnLineItem(line,log) {
  line.record['Customer_Partner_Logs__c'] = log; 
  line.record['Partner_Discount_Adjustment_Amount__c'] = 0;
...
//discount : discount to be applied
//type : type of discount applied
function reinitializeDiscountsOnLineItemRAF(line,log) {
  line.record['RAF_Logs__c'] = log;  
  line.record['RAF_Adjustment_Amount__c'] = 0;
  ...
  conn.query(query)
	.then(function(results) {
	  ...
	  lines.forEach(function(line) {
		...
		reinitializeDiscountsOnLineItem(line,'No Partner Discount Applied, No match found in Table');
		var obj;
		var log = '';
		var customerVSOEKey = line.record['HDSServiceSubType__c'] + line.record['VSOE_Discount__c'] + objQuote.End_User_Category__c;
		if (customerVSOEKey in mapCustomerDiscount1VSOE) {
