...
export function onInit(lines,conn) {
	...
	if(lines != null){
		...
		var query = "SELECT Id,SBQQ__Opportunity2__r.Install_At_Country__c,SBQQ__BillingCountry__c,Bill_To_Account__c,Bill_To_Account__r.SiteNumber__c,End_User_Category__c FROM SBQQ__Quote__c WHERE Id ='" +quoteId+ "' LIMIT 1" ;
		console.log(query);
		...
		.then(function(results) {
			if (results.totalSize) {
				...
				initRAFDiscounts(lines,conn,objQuote);
				initCustomerDiscountRule1(lines,conn,objQuote);
...
export function onBeforeCalculate(quoteModel, lines) {
	...
	var installAtCountry = quoteModel.record['SBQQ__BillingCountry__c'];
	var	billToAccount = quoteModel.record['Bill_To_Account__c'];
	//var	billToSite = quoteModel.record['Bill_To_Account__r']['SiteNumber__c '];
	//var endUserCategory = '';
	//console.log('Customer Discount:Install at:'+installAtCountry + ',Billtoaccount:'+billToAccount + ',billtosite:'+billToSite);
	lines.forEach(function(line) {
	
		/*
			creating the keys to validate the RAF discounts and apply them according to the orders or priority
			As Country is the first one we have our if on country and then region and then geo
		*/
		var geoKey = line.record['Geo__c'] + line.record['HDSServiceSubType__c'] + line.record['SBQQ__ProductFamily__c'];
		var regionKey = line.record['Region__c'] + line.record['HDSServiceSubType__c'] + line.record['SBQQ__ProductFamily__c'];
		var countryKey = installAtCountry + line.record['HDSServiceSubType__c'] + line.record['SBQQ__ProductFamily__c']; 
		var obj = null;
		if(countryKey in mapRAFCountry){
			obj = mapRAFCountry[countryKey];
		}else if(regionKey in mapRAFRegion){
			obj = mapRAFRegion[regionKey];
		}
		else if (geoKey in mapRAFGeo){
			obj = mapRAFGeo[geoKey];
		}
...
function initRAFDiscounts(lines,conn,objQuote){
		/*
			Create arrays of filters for RAF Lookup tables
			We have sub types, product families geo region and country as filters for lookup tables 
		*/
		var subTypes = [];
		var productFamilies = [];
		var geos = [];
		var regions = [];
		var installAtCountry;
		lines.forEach(function(line) {
			installAtCountry = line.record['SBQQ__Quote__r']['SBQQ__BillingCountry__c'];
			var subType = line.record['HDSServiceSubType__c'];
			var geo = line.record['Geo__c'];
			var region = line.record['Region__c'];
		...
		//creating raf queries with proper filters and selection fields that we need 
		var rafSelectQuery = 'SELECT Id,Adjustment_Amount__c,Adjustment_Type__c,Product__c,Geo__c,Region__c,Service_Sub_Type__c,Product_Family__c,Install_At_Country__c FROM Lookup_RAF__c ';
		var whereClause = " WHERE ";
		
		var andConditions = '';
		var orConditions = " (Install_At_Country__c ='" + installAtCountry + "') ";
		if(geos.length){
			var geoList = "('" + geos.join("', '") + "')";
			if(orConditions){
				orConditions = orConditions + ' OR ' + ' (Geo__c IN '+geoList+')'; 
			}else{
				orConditions = ' (Geo__c IN '+geoList+')';
			}
		}
		
		if(regions.length){
			var regionList = "('" + regions.join("', '") + "')";
			if(orConditions){
				orConditions = orConditions + ' OR ' + ' (Region__c IN '+regionList+')'; 
			}else{
				orConditions = '  (Region__c IN '+regionList + ')';
			}
		...
		if(subTypes.length){
			...
			andConditions += ' AND Service_Sub_Type__c IN '+subTypeList;
		}
		if(productFamilies.length){
			var productFamiliesList = "('" + productFamilies.join("', '") + "')";
			andConditions += ' AND Product_Family__c IN '+productFamiliesList;
		}
		
		//Iterating over all the results that was queried for RAF lookup and creating map
		var query = rafSelectQuery + whereClause + orConditions + andConditions;
		console.log('final raf query:'+query);
		return conn.query(query)
			.then(function(results) {
				if (results.totalSize) {
					results.records.forEach(function(record) {
						var objValue = {"discount":record.Adjustment_Amount__c,"type":record.Adjustment_Type__c};
						
						var geoKey = record.Geo__c + record.Service_Sub_Type__c + record.Product_Family__c;
						mapRAFGeo[geoKey] = objValue;
						var regionKey = record.Region__c + record.Service_Sub_Type__c + record.Product_Family__c;
						mapRAFRegion[regionKey] = objValue;
						var countryKey = record.Install_At_Country__c + record.Service_Sub_Type__c + record.Product_Family__c;
						mapRAFCountry[countryKey] = objValue;
...
function initCustomerDiscountRule1(lines,conn,objQuote){
	...
	conn.query(query)
		.then(function(results) {
			...
			console.log('mapCustomerDiscount1VSOE:'+mapCustomerDiscount1VSOE);
			var remainingLines = [];
			lines.forEach(function(line) {
				var obj;
...
function initCustomerDiscountRule2(lines,conn,objQuote){
	...
	console.log('Customer Discount2: bill to'+billToAccount+','+billToSite);
	/*for(var line of lines){
		billToAccount = line.record['SBQQ__Quote__r']['Bill_To_Account__c'];
		billToSite = line.record['SBQQ__Quote__r']['Bill_To_Account__r']['SiteNumber__c'];
	...
	conn.query(query)
		.then(function(results) {
			...
			lines.forEach(function(line) {
				...
				var productLinePricingCategoryKey = billToAccount  + billToSite + line.record['Product_Line__c'];// + line.record['Pricing_Category__c'];
				var productFamilySubTypeKey = billToAccount + billToSite + line.record['SBQQ__ProductFamily__c'] + line.record['HDSServiceSubType__c'];
				var productFamilyPricingCategoryKey = billToAccount + billToSite + line.record['SBQQ__ProductFamily__c'];// + line.record['Pricing_Category__c'];
...
function initCustomerDiscountRule3(lines,conn,objQuote){
	...
	var installAtCountry = objQuote.SBQQ__Opportunity2__r.Install_At_Country__c;
	lines.forEach(function(line) {
		var subType = line.record['HDSServiceSubType__c'];
	...
	conn.query(query)
		.then(function(results) {
			...
			lines.forEach(function(line) {
				...
				var regionProductLineKey = line.record['Region__c']  + line.record['Product_Line__c'] + line.record['HDSServiceSubType__c']+endUserCategory;
				var geoProductLineKey = line.record['Geo__c']  + line.record['Product_Line__c'] + line.record['HDSServiceSubType__c']+endUserCategory;
				...
				var regionProductFamilyKey = line.record['Region__c']  + line.record['SBQQ__ProductFamily__c'] + line.record['HDSServiceSubType__c']+endUserCategory;
				var geoProductFamilyKey = line.record['Geo__c']  + line.record['SBQQ__ProductFamily__c'] + line.record['HDSServiceSubType__c']+endUserCategory;
	...
	conn.query(query)
		.then(function(results) {
			...
			lines.forEach(function(line) {
				...
				var countryKey = installAtCountry  + line.record['SBQQ__ProductFamily__c'] +endUserCategory; //+ line.record['HDSServiceSubType__c']
				var regionKey = line.record['Region__c']  + line.record['SBQQ__ProductFamily__c'] + endUserCategory ;//+ line.record['HDSServiceSubType__c'] ;
				var geoKey = line.record['Geo__c']  + line.record['SBQQ__ProductFamily__c'] + endUserCategory;//+ line.record['HDSServiceSubType__c'] ;
...
function applyDiscountsOnLineItem(line,obj,ruleNumber){
	console.log(ruleNumber+':'+line.record['Id']);
	line.record['Customer_Discount_Adjustment_Amount__c'] = obj['discount'] || 0; 
	line.record['Customer_Discount_Applied__c'] = true;
	line.record['Customer_Discount_Adjustment_Type__c'] = obj['type'];
}