    ...
    return conn.query(query)
      .then(function(results) {
        if (results.totalSize) {
		  ...
		  //This is temporary condition will be updated later, to meet required criteria
		  console.log('Account Class : ' + objQuote.Account_Class__c + " :: Renewal_Contract_Package__r.Do_Not_Apply_Customer_Partner_Discount__c : " + objQuote.Renewal_Contract_Package__r.Do_Not_Apply_Customer_Partner_Discount__c);
			if(objQuote.Renewal_Contract_Package__r.Do_Not_Apply_Customer_Partner_Discount__c == false )
				if(objQuote.Account_Class__c == "Direct")
					...
					initPartnerDiscountRule1(lines, conn, objQuote);
...
function initRAFDiscounts(lines, conn, objQuote) {
  ...
  */
  ...
  var orConditions = '';
  if(installAtCountry)
	orConditions = " (Install_At_Country__c ='" + installAtCountry + "') ";
  if (geos.length) {
  ...
  lines.forEach(function(line) {
    ...
    */
    var geoKey = geo + line.record['HDSServiceSubType__c'] + line.record['SBQQ__ProductFamily__c'];
    var regionKey = region + line.record['HDSServiceSubType__c'] + line.record['SBQQ__ProductFamily__c'];
  ...
  conn.query(query)
    .then(function(results) {
      ...
      lines.forEach(function(line) {
        ...
        var regionKey = quoteRegion + line.record['SBQQ__ProductFamily__c'] + endUserCategory; //+ line.record['HDSServiceSubType__c'] ;
        var geoKey = quoteGeo + line.record['SBQQ__ProductFamily__c'] + endUserCategory; //+ line.record['HDSServiceSubType__c'] ;
...
function applyDiscountsOnLineItem(line, obj, ruleNumber) {
  console.log(ruleNumber + ': QLI Record ID :: ' + line.record['Id']);
...
// New Added code Prashant Gupta (in-Progress Work)
    ...
    .then(function(results) {
      ...
      lines.forEach(function(line) {
        ...
        var geoKey = quoteGeo + line.record['SBQQ__ProductFamily__c'] + line.record['HDSServiceSubType__c'] + partnerServiceCapablity;
			...
			console.log('>> LOG >> countryKey' + countryKey);
			console.log('>> LOG >> districtKey' + districtKey);
			console.log('>> LOG >> regionKey' + regionKey);
			console.log('>> LOG >> geoKey' + geoKey);
