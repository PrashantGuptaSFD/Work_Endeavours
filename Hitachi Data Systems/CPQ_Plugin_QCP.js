	  ...
	  if (results.totalSize) {
		results.records.forEach(function(record) {
		  ...
		  mapCustomerDiscount3Country[countryKey] = objValue;
			console.log('countryKey' + countryKey + record.Id);
			console.log('record.Install_At_Country__c' + record.Install_At_Country__c);
	...
	.then(function(results) {
	  ...
	  lines.forEach(function(line) {
		...
		if (regionKey in mapCustomerDiscount3Region) {
		  obj = mapCustomerDiscount3Region[regionKey];
		  log = 'Partner Rules Chunk 4 Rule 1 Applied [ Region = ' + quoteRegion + ' Product Family = '+line.record['Pricing_Product_Family__c']+ ' Product Type = '+ line.record['Product_Type__c'] + ' Partner Level = ' +partnerLevel+' ]';
		}
		else if (geoKey in mapCustomerDiscount3Geo) {
		  obj = mapCustomerDiscount3Geo[geoKey];
		  log = 'Partner Rules Chunk 4 Rule 1 Applied [ District = ' + quoteDistrict + ' Product Family = '+line.record['Pricing_Product_Family__c']+ ' Product Type = '+ line.record['Product_Type__c'] + ' Partner Level = ' +partnerLevel+' ]';
		}
		else if (districtKey in mapCustomerDiscount3Distict) {
		  obj = mapCustomerDiscount3Distict[districtKey];
